...
const ChatTabs = ({ activeTab, setActiveTab, notesCount, isLeadUnassigned, contactId, contactPhone }) => {
  ...
  return (
    <div className="bg-[#4a1015] border-b border-[#dea402]/20 px-5 flex-shrink-0">
      <div className="flex items-center gap-1">
        <button
          onClick={() => setActiveTab('chat')}
          className={`px-4 py-3 text-sm font-semibold transition-all duration-300 border-b-2 ${
            activeTab === 'chat'
              ? 'text-[#dea402] border-[#dea402]'
        ...
        >
          ...
          Notes
          {notesCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#dea402] text-black rounded-full text-xs flex items-center justify-center font-bold">
              {notesCount}
            </span>
          )}
        </button>
