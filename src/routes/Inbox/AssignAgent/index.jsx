...
const AssignAgent = ({ contact, refreshContacts }) => {
  ...
  return (
    <div className="p-5 space-y-4">
      {/* Current Assignment */}
      <div className="bg-[#5c1a20] rounded-xl border border-[#dea402]/20 p-4">
        <p className="text-xs text-gray-400 uppercase tracking-wider mb-2 font-semibold">Currently Assigned</p>
        {currentAgentId ? (
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#dea402] to-[#b38302] flex items-center justify-center text-black font-bold text-sm">
              {(() => {
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-[#5c1a20] border border-[#dea402]/20 rounded-xl text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#dea402]/50 transition-colors"
        />
      </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ${
                    isCurrentAgent
                      ? 'bg-[#dea402]/10 border border-[#dea402]/30 cursor-default'
                      : 'bg-[#5c1a20] border border-transparent hover:border-[#dea402]/30 hover:bg-[#5c1a20]/80'
                  }`}
                >
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                    isCurrentAgent
                      ? 'bg-gradient-to-br from-[#dea402] to-[#b38302] text-black'
                      : 'bg-[#4a1015] text-[#dea402] border border-[#dea402]/30'
                  }`}>
                    {agent.firstName?.[0]?.toUpperCase() || '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate ${isCurrentAgent ? 'text-[#dea402]' : 'text-white'}`}>
                      {agent.firstName} {agent.lastName}
                    </p>
                    <p className="text-xs text-gray-400 truncate">{agent.roleName || 'Agent'}</p>
                  </div>
                  {isAssigning ? (
                    <Loader2 className="w-4 h-4 text-[#dea402] animate-spin flex-shrink-0" />
                  ) : isCurrentAgent ? (
                    <span className="text-xs text-[#dea402] font-medium flex-shrink-0">Assigned</span>
                  ) : (
                    <UserCheck className="w-4 h-4 text-gray-500 group-hover:text-[#dea402] flex-shrink-0" />
                  )}
                </button>
              );
            })}
