  ...
  return (
    <div className="flex items-center gap-2.5 min-w-[220px]">
      ...
      <button
        ...
        className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 active:scale-90 ${
          isUserMessage
            ...
            : 'bg-[#dea402]/20 hover:bg-[#dea402]/30 text-[#dea402]'
        }`}
      >
        {isPlaying
            ...
            return (
              <div
                ...
                className={`flex-1 rounded-full transition-colors duration-150 ${
                  isFilled
                    ? isUserMessage ? 'bg-black/60' : 'bg-[#dea402]'
                    : isUserMessage ? 'bg-black/15' : 'bg-[#dea402]/25'
                }`}
                style={{ height: `${h}px` }}
              />
            );
      ...
      <div className="relative flex-shrink-0" ref={audioMenuRef}>
        <button
          ...
          className={`min-w-[32px] h-7 rounded-lg flex items-center justify-center text-[10px] font-bold transition-all ${
            isUserMessage
              ...
              : 'bg-[#dea402]/15 hover:bg-[#dea402]/25 text-[#dea402]'
          }`}
        ...
        {showAudioMenu && (
          <div className={`absolute right-0 bg-[#5c1a20] border border-[#dea402]/20 rounded-xl shadow-2xl py-1.5 min-w-[130px] z-40 animate-scaleIn ${
            ...
            {speeds.map(s => (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-1.5 text-sm transition-colors ${
                  playbackRate === s
                    ? 'text-[#dea402] bg-[#dea402]/10'
                    : 'text-gray-200 hover:bg-[#dea402]/10 hover:text-[#dea402]'
                }`}
              >
                <span>{s}x</span>
                {playbackRate === s && <span className="text-[#dea402] text-xs">&#10003;</span>}
              </button>
            ))}
            <div className="my-1 border-t border-[#dea402]/10" />
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (src) {
              ...
              className="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-gray-200 hover:bg-[#dea402]/10 hover:text-[#dea402] transition-colors"
            >
...
const MessagesArea = ({
  ...
  const renderMessageContent = (message) => {
    ...
    if (message.type === 'image' && !message.mediaUrl) {
      return (
        <div className="flex items-center gap-2 py-2 px-1">
          <ImageIcon className="w-5 h-5 text-[#dea402]" />
          <span className="text-sm text-[#dea402]/80">{message.text || 'Image'}</span>
        </div>
      );
    }
    if ((message.type === 'image' || (message.type === 'text' && message.mediaUrl)) && message.mediaUrl) {
      ...
      return (
        <div className="space-y-2">
          {hasFailed ? (
            <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl min-h-[150px] max-w-[300px]">
              <svg className="w-12 h-12 text-[#dea402]/50 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p className="text-[#dea402]/70 text-sm">Image preview</p>
              <button
                onClick={() => handleMediaDownload(message)}
                className="mt-3 px-3 py-1.5 bg-[#dea402]/20 hover:bg-[#dea402]/30 rounded-lg text-[#dea402] font-medium text-xs transition-all"
              >
                Retry
              </button>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl min-h-[150px] max-w-[300px]">
              <Loader2 className="w-12 h-12 text-[#dea402] animate-spin mb-2" />
              <p className="text-[#dea402]/70 text-sm">Loading image...</p>
            </div>
          )}
    ...
    if (message.type === 'audio' && !message.mediaUrl) {
      return (
        <div className="flex items-center gap-2 py-2 px-1">
          <Mic className="w-5 h-5 text-[#dea402]" />
          <span className="text-sm text-[#dea402]/80">{message.text || 'Voice Note'}</span>
        </div>
      );
    }
      ...
      return (
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <Mic className={`w-4 h-4 flex-shrink-0 ${isUser ? 'text-black/50' : 'text-[#dea402]'}`} />
            <div className="flex-1 min-w-0">
              {hasFailed ? (
                ...
                <div className={`flex items-center gap-2 ${isUser ? 'text-black/50' : 'text-[#dea402]'}`}>
    ...
    if (message.type === 'video' && !message.mediaUrl) {
      return (
        <div className="flex items-center gap-2 py-2 px-1">
          <Video className="w-5 h-5 text-[#dea402]" />
          <span className="text-sm text-[#dea402]/80">{message.text || 'Video'}</span>
        </div>
      );
    }
    // VIDEO DISPLAY
    if (message.type === 'video' && message.mediaUrl) {
      ...
      return (
        <div className="space-y-2">
          {hasFailed ? (
            <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl min-h-[150px] max-w-[300px]">
              <Video className={`w-12 h-12 mb-2 ${isUser ? 'text-black/30' : 'text-[#dea402]/50'}`} />
              <p className={`text-sm ${isUser ? 'text-black/50' : 'text-[#dea402]/70'}`}>Failed to load video</p>
              <button
                onClick={() => handleMediaDownload(message)}
                className="mt-3 px-3 py-1.5 bg-[#dea402]/20 hover:bg-[#dea402]/30 rounded-lg text-[#dea402] font-medium text-xs transition-all"
              >
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl min-h-[150px] max-w-[300px]">
              <Loader2 className="w-12 h-12 text-[#dea402] animate-spin mb-2" />
              <p className="text-[#dea402]/70 text-sm">Loading video...</p>
            </div>
          )}
    ...
    if (message.type === 'document' && !message.mediaUrl) {
      return (
        <div className="flex items-center gap-2 py-2 px-1">
          ...
          <span className="text-sm text-[#dea402]/80">{message.text || 'Document'}</span>
        </div>
      );
    }
    ...
    if (message.type === 'document' && message.mediaUrl) {
      ...
      return (
        <div className="space-y-2">
          {hasFailed ? (
            <div className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl min-h-[100px] max-w-[280px]">
              <FileText className={`w-10 h-10 mb-2 ${isUser ? 'text-black/30' : 'text-[#dea402]/50'}`} />
              <p className={`text-sm ${isUser ? 'text-black/50' : 'text-[#dea402]/70'}`}>Failed to load document</p>
              <button
                onClick={() => handleMediaDownload(message)}
                className="mt-3 px-3 py-1.5 bg-[#dea402]/20 hover:bg-[#dea402]/30 rounded-lg text-[#dea402] font-medium text-xs transition-all"
              >
            ...
            <div
              className={`flex items-center gap-3 p-3 rounded-xl max-w-[280px] cursor-pointer transition-all hover:opacity-80 ${
                isUser ? 'bg-black/10' : 'bg-[#dea402]/10'
              }`}
            ...
            >
              ...
              <Download className={`w-4 h-4 flex-shrink-0 ${isUser ? 'text-black/40' : 'text-[#dea402]/60'}`} />
            </div>
          ) : (
            <div className="flex items-center gap-3 p-3 bg-gradient-to-br from-[#dea402]/10 to-[#b38302]/10 rounded-xl max-w-[280px]">
              <Loader2 className="w-5 h-5 text-[#dea402] animate-spin flex-shrink-0" />
              <span className="text-[#dea402]/70 text-sm">Loading document...</span>
            </div>
          )}
      ...
      return (
        <div className="rounded-2xl px-4 py-3 shadow-md bg-gradient-to-r from-[#005C4B] to-[#128C7E] text-white border border-[#25D366]/30 transition-all duration-300 hover:shadow-lg hover:scale-[1.02] min-w-[200px]">
          {message.templateName && (
            <div className={`flex items-center gap-1.5 mb-2 pb-2 border-b ${isUser ? 'border-white/20' : 'border-[#dea402]/20'}`}>
              <FileText className={`w-3.5 h-3.5 ${isUser ? 'text-white/70' : 'text-[#dea402]'}`} />
              <span className={`text-xs font-semibold uppercase tracking-wide ${isUser ? 'text-white/70' : 'text-[#dea402]'}`}>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-6">
        <div className="relative">
          <div className="animate-spin rounded-full h-14 w-14 border-4 border-[#dea402]/20 border-t-[#dea402]"></div>
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#dea402] to-[#b38302]"></div>
          </div>
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center animate-fadeIn p-6">
        <div className="relative mb-6">
          <div className="w-28 h-28 rounded-full bg-gradient-to-br from-[#dea402]/20 to-[#b38302]/20 flex items-center justify-center animate-pulse-slow">
            <svg className="w-14 h-14 text-[#dea402]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="relative p-6 space-y-3">
      ...
      {!mediaProgress.isComplete && (
        <div className="sticky top-0 z-20 flex justify-center mb-2">
          <div className="bg-[#5c1a20]/95 backdrop-blur-sm px-5 py-2.5 rounded-full border border-[#dea402]/30 shadow-xl flex items-center gap-3">
            <Loader2 className="w-4 h-4 text-[#dea402] animate-spin" />
            <span className="text-sm text-gray-300">
              Loading media {mediaProgress.downloaded}/{mediaProgress.total}
            </span>
            <div className="w-20 h-1.5 bg-[#4a1015] rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#dea402] to-[#b38302] rounded-full transition-all duration-300"
      ...
      {pinnedMessages?.length > 0 && (
        <div className="sticky top-0 z-20 mb-2 space-y-1.5">
          {pinnedMessages.map(pin => (
            <div
              key={pin.id}
              className="bg-[#5c1a20]/95 backdrop-blur-sm px-4 py-2 rounded-xl border border-[#dea402]/30 shadow-xl cursor-pointer hover:bg-[#333]/95 transition-colors"
              onClick={() => {
            ...
            >
              <div className="flex items-center gap-2.5">
                <Pin className="w-3.5 h-3.5 text-[#dea402] flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-semibold text-[#dea402]">Pinned</p>
      ...
      {Object.keys(messageGroups).map((dateKey) => (
        <div key={dateKey} className="space-y-3">
          ...
          <div className="sticky top-0 z-10 flex items-center justify-center py-2">
            <div className="bg-[#5c1a20] px-4 py-1.5 rounded-full text-xs font-semibold text-gray-400 border border-[#dea402]/20 shadow-lg backdrop-blur-sm">
              {dateKey}
            </div>
          </div>
          ...
          {messageGroups[dateKey].map((message) => (
            ...
            >
              <div className={`max-w-[80%] group ${message.sender === 'user' || (message.isTemplate && message.text) ? 'items-end' : 'items-start'} flex flex-col`}>
                {message.isTemplate && (
                  <div className={`flex items-center gap-1 mb-1 text-xs ${message.sender === 'user' || (message.isTemplate && message.text) ? 'text-[#dea402]' : 'text-gray-400'}`}>
                    ...
                    className={`rounded-2xl px-4 py-3 shadow-md ${message.sender === 'user'
                      ? message.failed
                        ...
                        : message.isTemplate
                          ...
                          : 'bg-gradient-to-r from-[#dea402] to-[#b38302] text-black'
                      : 'bg-[#5c1a20] text-white border border-[#dea402]/20'
                    ...
                    {message.replyTo && (
                      ...
                      >
                        <p className={`text-[11px] font-semibold ${
                          message.sender === 'user' ? 'text-black/60' : 'text-[#dea402]'
                        }`}>
                  ...
                  {(message.sender !== 'user' && !(message.isTemplate && message.text)) &&  (
                    <div className="relative flex-shrink-0" ref={activeMenuId === message.id ? menuRef : undefined}>
                      ...
                      {activeMenuId === message.id && (
                        <div className={`absolute left-0 bg-[#5c1a20] border border-[#dea402]/20 rounded-xl shadow-2xl py-1.5 min-w-[160px] z-30 animate-scaleIn ${
                          menuOpenUp ? 'bottom-full mb-1' : 'top-full mt-1'
                        }`}>
                          <button
                            onClick={() => { onReply?.(message); setActiveMenuId(null); }}
                            className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-gray-200 hover:bg-[#dea402]/10 hover:text-[#dea402] transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-gray-200 hover:bg-[#dea402]/10 hover:text-[#dea402] transition-colors"
                          >
                            <Pin className={`w-3.5 h-3.5 ${pinnedMessages?.some(p => p.id === message.id) ? 'text-[#dea402]' : ''}`} />
