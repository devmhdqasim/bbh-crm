...
/**
 * Get agent lead stats for sales manager
 * @param {string} agentId - Agent ID/username to fetch stats for
 * @returns {Promise} - Returns agent lead statistics
 */
export const getAgentLeadStats = async (agentId = '') => {
  try {
    const authToken = getRefreshToken();
    ...
    const response = await axios.get(
      `${API_BASE_URL}/lead/status-stats/en?agentId=${agentId}`,
      {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`,
    ...
    if (data.status === 'success' && data.payload) {
      ...
      return {
        success: false,
        message: data.message || 'Failed to fetch agent lead stats',
        data: null,
      };
    }
  } catch (error) {
    ...
    if (error.response) {
      return {
        success: false,
        message: error.response.data?.message || 'Failed to fetch agent lead stats',
        error: error.response.data,
      };
    } else if (error.request) {
      return {
